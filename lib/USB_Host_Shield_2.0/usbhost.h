...
#elif defined(ESP32)
// Omni Chair, SPI pins must match src/configs/pins.h
typedef SPi< P13, P11, P10, P18 > spi;
...
uint16_t MAX3421e< SPI_SS, INTR >::reset() {
        ...
        while(++i) {
                ...
                // Feed the watchdog while polling - with no shield connected this
                // loop runs the full 65535 iterations, which otherwise starves the
                // interrupt watchdog and resets the chip (ESP32-S3 boards without a
                // USB Host Shield attached). delay(), unlike yield(), actually blocks
                // for a tick and guarantees the scheduler moves on.
                if((i & 0x3F) == 0) delay(1);
        }
        return ( i);
}
...
int8_t MAX3421e< SPI_SS, INTR >::Init() {
        ...
        // Bounded and watchdog-fed like reset() above - this was previously an
        // unbounded loop that could starve the interrupt watchdog if the bus
        // never reports sampling done.
        for (uint16_t Sample_Wait = 0; !(regRd(rHCTL) & bmSAMPLEBUS); Sample_Wait++) {
                if(Sample_Wait >= 65535) break;
                if((Sample_Wait & 0x3F) == 0) delay(1);
        }
